...
class ReplayBuffer:  # for off-policy
    def __init__(self,
        ...
        self.b_obs_edges = torch.empty((max_size, num_seqs, 2, num_edges), dtype=torch.long, device=self.device)
        self.b_obs_nodes = torch.empty((max_size, num_seqs, num_points, 4), dtype=torch.float32, device=self.device)
        self.b_obs_pos = torch.empty((max_size, num_seqs, state_dim), dtype=torch.float32, device=self.device)
        ...
        self.if_use_per = if_use_per    # Trick3
        if if_use_per:
    ...
    def update(self, items: Tuple[Tensor, ...]):
        ...
        h_obs_edges, h_obs_nodes, h_obs_pos, actions, rewards, undones, action_masks = items
        ...
        if p > self.max_size:
            ...
            self.b_obs_edges[p0:p1], self.b_obs_edges[0:p] = h_obs_edges[:p2], h_obs_edges[-p:]
            self.b_obs_nodes[p0:p1], self.b_obs_nodes[0:p] = h_obs_nodes[:p2], h_obs_nodes[-p:]
            self.b_obs_pos[p0:p1], self.b_obs_pos[0:p] = h_obs_pos[:p2], h_obs_pos[-p:]
            ...
            self.b_obs_edges[self.p:p] = h_obs_edges
            self.b_obs_nodes[self.p:p] = h_obs_nodes
            self.b_obs_pos[self.p:p] = h_obs_pos
    ...
    def sample(self, batch_size: int) -> Tuple[Tensor, Tensor, Tensor, Tensor, Tensor, Tensor]:
        ...
        return (self.b_obs_edges[ids0, ids1],
                self.b_obs_nodes[ids0, ids1],
                self.b_obs_pos[ids0, ids1],
                self.actions[ids0, ids1],
                self.rewards[ids0, ids1],
                self.undones[ids0, ids1],
                self.b_obs_edges[ids0 + 1, ids1],
                self.b_obs_nodes[ids0 + 1, ids1],
                self.b_obs_pos[ids0 + 1, ids1],
                self.action_masks[ids0, ids1],
                )
