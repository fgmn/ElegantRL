...
class Evaluator:
    ...
    def evaluate_and_save(self, actor: torch.nn, steps: int, exp_r: float, logging_tuple: tuple):
        ...
        print(f"{self.agent_id:<3}{self.total_step:8.2e}{train_time:8.0f} |"
              ...
              f"{exp_r:8.2f}{''.join(f'{n:15.2f}' for n in logging_tuple)}")
...
def get_cumulative_rewards_and_steps(env, actor, if_render: bool = False) -> Tuple[float, int]:
    ...
    ary_obs_edges, ary_obs_nodes, ary_obs_pos, action_mask = env.reset()
    steps = None
    returns = 0.0  # sum of rewards in an episode
    for steps in range(max_step):
        # tensor_state = torch.as_tensor(state, dtype=torch.float32, device=device).unsqueeze(0)
        obs_edges = torch.tensor(ary_obs_edges, dtype=torch.long, device=device).unsqueeze(0)
        obs_nodes = torch.tensor(ary_obs_nodes, dtype=torch.float32, device=device).unsqueeze(0)
        obs_pos = torch.tensor(ary_obs_pos, dtype=torch.float32, device=device).unsqueeze(0)
        # tensor_action = actor(tensor_state)
        tensor_action = actor(obs_edges, obs_nodes, obs_pos)
        ...
        ary_obs_edges, ary_obs_nodes, ary_obs_pos, reward, done, _ , action_mask = env.step(action)  # next_state
        returns += reward
