...
# 创建带有时间戳的日志目录
log_dir = f"logs/dqn/{current_time}"
...
for episode in tqdm(range(1, episodes + 1), desc="Episodes", leave=True):
    ...
    for step in tqdm(range(max_steps), desc=f"Episode {episode}", leave=False):
        ...
        # action = min(agent.take_action(current_state), len(neighbor_c)-1)   # fixd
        i_current_state = [important_points[current_state, 0], important_points[current_state, 1]]
        action = agent.take_action(i_current_state, len(neighbor_c))
        n_state = neighbor_c[action]
        i_n_state = [important_points[n_state, 0], important_points[n_state, 1]]

        # Calculate the distance
        dis = np.linalg.norm(important_points[current_state] - important_points[n_state])
        distance_weight[current_state, action] = dis
        ...
        reward2 = (88 * (deta / abs(deta)) if deta != 0 else 0)
        ...
        replay_buffer.add(i_current_state, action, reward, i_n_state, done, len(neighbor_c))
        
        # 当buffer数据的数量超过一定值后,才进行Q网络训练
        if replay_buffer.size() > minimal_size:
            b_s, b_a, b_r, b_ns, b_d, b_ne = replay_buffer.sample(batch_size)
            transition_dict = {
                'states': b_s,
                'actions': b_a,
                'next_states': b_ns,
            ...
            #     summary.scalar("Critic Loss(Step)", episode_loss, step=(episode-1) * max_steps + step)
    ...
    with writer.as_default():
        ...
        summary.scalar("Critic Loss(Episode)", episode_loss, step=episode)
########################## DQN TRAIN ###########################
